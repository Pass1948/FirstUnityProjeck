...
public class UnityMessageEvent : MonoBehaviour
{
	 ...
	 * 유니티메시지 이벤트함수
	 * 
	 * 유니티가 보내는 메시지에 반응하는 함수(메세지 브로드캐시팅방식)
	 * 포함할 경우 유니티메시지에 반응하지만, 포함하지 않을 경우 무시함
	 * 스크립트는 유니티 엔진이 보내는 메시지에 반응하여 자신의 행동을 정의
	 ************************************************************************/

    private void Awake()            // 첫 초기화 당시 설정하는 함수
        // start보다 먼저 실행되는 함수로 게임의 필요한 상황들을 준비하는 함수
    {
    ...
    private void Start()
        // 모든 준비가 완료된 상황에서 게임이 실행되기 직전에 실행되는 함수
    {
        // 스크립트가 씬에 처음으로 Update하기 직전에 1회 호출됨
        // 스크립트가 비활성화된 경우 호출되지 않음 (도중 업데이트, 참가 된 상황)
    ...
    private void LateUpdate()
    {
        ...
        // 역할 : 게임프레임의 진행 결과가 필요한 동작이 있는 기능 구현  (판정류가 필요한 상황에 쓰임)
